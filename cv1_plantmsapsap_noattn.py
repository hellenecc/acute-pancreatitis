...
from mymodel_vgg_two_class_no_attn import VGG
...
def main():
    ...
    for fold in range(k):
        ...
        for valfile in valpath[fold]:
            valx.append(valfile)
            if 'level2' in valfile.split('/')[-1]:
                label = 0
            elif 'level3' in valfile.split('/')[-1]:
                label = 1
            valy.append(label)
        ...
        for trainfile in trainpath[fold]:
            ...
            if 'level2' in trainfile.split('/')[-1]:
                label = 0
            elif 'level3' in trainfile.split('/')[-1]:
                label = 1
            trainy.append(label)
        ...
        net = VGG(im_size=im_size, num_classes=2, init='xavierUniform')
