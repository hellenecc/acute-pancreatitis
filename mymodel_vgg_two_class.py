...
class AttnVGG(nn.Module):
    def __init__(self, im_size, num_classes, attention=True, normalize_attn=True, init='xavierUniform'):
        ...
        self.linear1=nn.Linear(in_features=1536, out_features=30, bias=True)
        #self.linear2 = nn.Linear(in_features=1568, out_features=400, bias=True)

        # Projectors & Compatibility functions
        if self.attention:
            #self.projector = ProjectorBlock(256, 512)
            self.attn1 = GridAttentionBlock(in_features_l=512,in_features_g=512,attn_features=512, up_factor=4,normalize_attn=normalize_attn)
            self.attn2 = GridAttentionBlock(in_features_l=512,in_features_g=512,attn_features=512, up_factor=2,normalize_attn=normalize_attn)
            self.attn3 = GridAttentionBlock(in_features_l=512,in_features_g=512,attn_features=512, up_factor=1,normalize_attn=normalize_attn)
        # final classification layer
        if self.attention:
            self.classify = nn.Linear(in_features=1536, out_features=2, bias=True)
        else:
