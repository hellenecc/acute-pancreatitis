...
from mymodel_vgg_no_attn import VGG
...
def main():
    ...
    for fold in range(k):
        ...
        trainloader = t.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
        valloader = t.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=0)

        net = VGG(im_size=im_size, num_classes=3, init='xavierUniform')
